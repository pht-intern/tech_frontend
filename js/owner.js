...
// Order for itemsTypeFilters: from database (quotationTypeFilters); fallback order for sort when DB empty
...
async function renderItemsTypeFilters() {
    ...
    // Build ordered list: All, then DB order (only include types that exist in items or in DB for future items), then types from items not in DB
    const orderLower = orderFromDb.map(function (t) { return String(t).toLowerCase().trim(); });
    const typesFromItemsLower = new Set(typesFromItems.map(function (t) { return String(t).toLowerCase().trim(); }));
    const orderedPairs = [{ label: 'All', value: '' }];
...
async function renderLogsList() {
    ...
    filterLogsTable();
    if (!window._ownerLogsSearchListenerAttached) {
        window._ownerLogsSearchListenerAttached = true;
...
async function renderSettings() {
    ...
    var sizePrimaryEl = document.getElementById('settings-pdf-font-size-primary');
    var sizeSecondaryEl = document.getElementById('settings-pdf-font-size-secondary');
    var sizeTertiaryEl = document.getElementById('settings-pdf-font-size-tertiary');
    if (sizePrimaryEl) sizePrimaryEl.value = getEffectivePdfFontSizePrimary();
    if (sizeSecondaryEl) sizeSecondaryEl.value = getEffectivePdfFontSizeSecondary();
    if (sizeTertiaryEl) sizeTertiaryEl.value = getEffectivePdfFontSizeTertiary();
...
function renumberQuotationItemsOrderList() {
    ...
    items.forEach(function (li, idx) {
        ...
        if (spans.length > 0) spans[0].textContent = (idx + 1) + '.'; // first span is sl no
    });
}
...
async function convertItemDraft(draftId) {
    try {
        ...
        addLog('Product Created from Draft', CURRENT_USER_ROLE, `Converted product draft: ${draftId}`);
...
async function showSection(sectionId) {
    ...
    if (sectionId === 'createQuotation') {
        ...
        loadQuotationDrafts();
        const createBtn = document.getElementById('createQuotationBtn');
        const createTabLabel = 'Quotations';
...
function getCustomFontsCssForPdf() {
    var primary = getEffectivePdfFontPrimary();
    var secondary = getEffectivePdfFontSecondary();
    var tertiary = getEffectivePdfFontTertiary();
    var keys = [primary, secondary, tertiary];
    var custom = getCustomPdfFonts();
    var used = {};
    ...
    custom.forEach(function (f) {
        ...
        var name = (f.name || 'CustomFont').replace(/'/g, "\\'");
        var url = (origin + f.url).replace(/'/g, "\\'");
        var format = f.url.indexOf('.woff2') !== -1 ? 'woff2' : f.url.indexOf('.woff') !== -1 ? 'woff' : f.url.indexOf('.otf') !== -1 ? 'opentype' : 'truetype';
...
function refreshPdfCustomFontOptions() {
    var custom = getCustomPdfFonts();
    var styleId = 'pdf-custom-fonts-style';
    ...
    var listEl = document.getElementById('pdf-custom-fonts-list');
    if (listEl) {
        if (custom.length === 0) {
            listEl.innerHTML = '';
        } else {
...
document.getElementById('uploadPdfFontBtn')?.addEventListener('click', function () {
    var input = document.getElementById('settings-pdf-font-upload');
    ...
    apiFetch('/upload-font', { method: 'POST', body: formData })
        .then(function (res) {
            ...
            if (data && data.url) {
                var custom = getCustomPdfFonts();
                var id = 'custom_' + (Date.now().toString(36) + Math.random().toString(36).slice(2));
...
document.getElementById('savePdfFontBtn')?.addEventListener('click', function () {
    ...
    var sizePrimary = document.getElementById('settings-pdf-font-size-primary');
    var sizeSecondary = document.getElementById('settings-pdf-font-size-secondary');
    var sizeTertiary = document.getElementById('settings-pdf-font-size-tertiary');
    ...
    try {
        ...
        var p = parseInt(sizePrimary && sizePrimary.value ? sizePrimary.value : 26, 10);
        var s = parseInt(sizeSecondary && sizeSecondary.value ? sizeSecondary.value : 14, 10);
        var t = parseInt(sizeTertiary && sizeTertiary.value ? sizeTertiary.value : 12, 10);
